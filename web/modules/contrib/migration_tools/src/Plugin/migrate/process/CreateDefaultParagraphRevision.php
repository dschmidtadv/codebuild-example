...
class CreateDefaultParagraphRevision extends ProcessPluginBase {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    if ($bundle) {
      return $this->createParagraphRevision($bundle, $paragraph_data, $row);
    }
   ...
   *   Row data for pulling out token values.
   *
   * @return int|null
   *   The paragraph id if one was created.  NULL otherwise.
   */
  protected function createParagraphRevision(string $bundle, array $field_values, Row $row) {
    ...
    if ($bundle) {
      foreach ($field_values as $field_name => $value) {
        if ($bundle_new = $this->getParagraphBundleToBuild($value)) {
          ...
          $field_values[$field_name] = $this->createParagraphRevision($bundle_new, $value, $row);
        }
        $field_values[$field_name] = $this->optionallyGrabMatchingTokenValue($value, $row);
      }
   ...
   *
   * @param mixed $value
   *   The value of an entry in the paragraph_default config.
   * @param \Drupal\migrate\Row $row
   *   The migration row.
   *
   * @return mixed
